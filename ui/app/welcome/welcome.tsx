...
interface ChangeTransferPolicy {
  metadata: {
    name: string;
    namespace?: string;
    labels?: Record<string, string>;
...
export function ChangeTransferPoliciesList() {
  const [policies, setPolicies] = useState<ChangeTransferPolicy[]>([]);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    async function fetchPolicies() {
      try {
        const response = await fetch("/list?kind=changetransferpolicy");
        if (!response.ok) {
          throw new Error(`Error: ${response.statusText}`);
        }
        const data = await response.json();
        setPolicies(data || []); // Get items from response
      } catch (err: any) {
  ...
  return (
      <div className="change-transfer-policies">
        <h2 className="text-xl font-bold mb-4">Change Transfer Policies</h2>
        <ul className="space-y-2">
          {policies.map((policy) => (
              <li key={policy.metadata.namespace + "/" + policy.metadata.name} className="p-4 border rounded shadow">
                <h3 className="text-lg font-semibold">{policy.metadata.namespace + "/" + policy.metadata.name}</h3>
                <p className="text-gray-600">
                  Repository: {policy.spec.gitRepositoryRef.name}
                </p>
                <p className="text-gray-600">
                  Proposed Branch: {policy.spec.proposedBranch}
                </p>
                <p className="text-gray-600">
                  Active Branch: {policy.spec.activeBranch}
                </p>
                <p className="text-gray-600">
                ...
                {policy.status && (
                    <div className="mt-2">
                      <p className="text-gray-600">
                        Proposed Dry SHA: {policy.status.proposed?.dry?.sha || "N/A"}
                      </p>
                      <p className="text-gray-600">
                        Active Dry SHA: {policy.status.active?.dry?.sha || "N/A"}
                      </p>
                    </div>
                )}
              </li>
...
export function ChangeTransferPoliciesListUpdate() {
  const [policies, setPolicies] = useState<ChangeTransferPolicy[]>([]);
  const [error, setError] = useState<string | null>(null);
    ...
    eventSource.addEventListener("ChangeTransferPolicy", event => {
      try {
        ...
        setPolicies((currentPolicies) => {
          ...
          if (index >= 0) {
            updatedPolicies[index] = message;
          } else {
            updatedPolicies.push(message);
          }
  ...
  return (
    <main className="flex items-center justify-center pt-16 pb-4">
      <div className="flex-1 flex flex-col items-center gap-16 min-h-0">
        ...
        <PromotionStrategiesList />
        <ChangeTransferPoliciesListUpdate />
      </div>
    </main>
  );
}
