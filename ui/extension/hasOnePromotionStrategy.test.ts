...
import { hasOnePromotionStrategy, ANNOTATION } from "./hasOnePromotionStrategy";
...
describe("hasOnePromotionStrategy", () => {
  describe("annotation-based lookup", () => {
    it("returns true when the annotation is present", () => {
      const app = makeApp({
        metadata: {
          name: "my-app",
          namespace: "default",
          annotations: { [ANNOTATION]: "my-ns/my-ps" },
        },
      });
      expect(hasOnePromotionStrategy(app)).toBe(true);
    });
    ...
    it("returns true when annotation is present even with multiple PromotionStrategy resources", () => {
      const app = makeApp({
        ...
        status: {
          resources: [
            { kind: "PromotionStrategy", group: "promoter.argoproj.io", name: "ps1", namespace: "ns1", status: "Synced" },
            { kind: "PromotionStrategy", group: "promoter.argoproj.io", name: "ps2", namespace: "ns2", status: "Synced" },
          ],
        },
      });
      expect(hasOnePromotionStrategy(app)).toBe(true);
  ...
  describe("tree-based fallback (no annotation)", () => {
    ...
    it("returns false when there are no resources", () => {
      ...
      expect(hasOnePromotionStrategy(app)).toBe(false);
    });
    ...
    it("returns false when there are no PromotionStrategy resources (only other kinds)", () => {
      const app = makeApp({
        status: {
          resources: [
            { kind: "Deployment", group: "apps", name: "deploy", namespace: "ns1", status: "Synced" },
          ],
        },
      });
      expect(hasOnePromotionStrategy(app)).toBe(false);
    ...
    it("returns false when there are multiple PromotionStrategy resources", () => {
      const app = makeApp({
        status: {
          resources: [
            { kind: "PromotionStrategy", group: "promoter.argoproj.io", name: "ps1", namespace: "ns1", status: "Synced" },
