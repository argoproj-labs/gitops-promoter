...
type GitCommitStatusSpec struct {
	...
	//
	// Most use cases want "active" to validate the current state before allowing promotion.
	// Use "proposed" when you want to validate the incoming change itself (e.g., commit message format,
	// author restrictions, etc.).
	//
...
// - "active" (default): Validates the currently deployed commit (state-based gating)
// - "proposed": Validates the incoming commit (change-based gating)
//
// The validation result is always reported on the PROPOSED commit to enable promotion gating,
