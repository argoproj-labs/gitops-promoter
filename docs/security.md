...
**Repository identity required.** The receiver only processes payloads that include a parseable repository identity (provider-specific fields such as GitHub `repository.full_name` / `repository.owner.login` + `repository.name`, GitLab `project.path_with_namespace`, Bitbucket Cloud `repository.full_name`, or Azure DevOps `resource.repository`). Org/group-only and other events that omit that identity are not supported for ChangeTransferPolicy or WebRequestCommitStatus enqueue. If any ScmProvider / ClusterScmProvider Secret has `webhookSecret` configured and a delivery lacks repository identity, the receiver cannot verify the request and returns **401** (fail closed) so neither path is enqueued.
