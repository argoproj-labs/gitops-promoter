...
- **Commit status controllers** — If you create or update `CommitStatus` objects, follow [Developing a CommitStatus](developing-a-commitstatus.md) (standard labels, owner references, keys that match `PromotionStrategy` config, and `promoter.argoproj.io/instance-id` stamping from the controller’s configured `instanceID`).
- **Multi-install partitioning** — In-repo reconcilers that create child Promoter CRs must stamp `promoter.argoproj.io/instance-id` via `utils.StampInstanceIDLabel` / `CommitStatusStandardLabels` (see [Developing a CommitStatus](developing-a-commitstatus.md) and [Multiple Controller Installs](../multi-install.md)). Do not wire per-controller instance-id list predicates; the manager cache already partitions by label.
...
- **UI plugins** — Commit status row plugins, authored outside this repo and loaded into the dashboard and/or Argo CD extension, follow [Developing UI Plugins](developing-ui-plugins.md).
