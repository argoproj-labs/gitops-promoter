...
- **Commit status controllers** — If you create or update `CommitStatus` objects, follow [Developing a CommitStatus](developing-a-commitstatus.md) (standard labels, owner references, keys that match `PromotionStrategy` config, and `promoter.argoproj.io/instance-id` propagation when the parent gate carries it).
- **Multi-install partitioning** — In-repo reconcilers that create child Promoter CRs must propagate `promoter.argoproj.io/instance-id` from the parent (see [Developing a CommitStatus](developing-a-commitstatus.md) and [Multiple Controller Installs](../multi-install.md)). Built-in enforcement is cache `ByObject` at manager startup; contributors do not wire per-controller instance-id predicates.
