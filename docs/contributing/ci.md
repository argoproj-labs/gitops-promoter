...
- **Codegen verification** — ensures `go.sum`, mockery output (`internal/scms/mock/`), and `make build-installer` output (CRDs, `applyconfiguration/`, deepcopy, extension icon styles, `hack/celcost/report.md`, `dist/install.yaml`) are up to date
