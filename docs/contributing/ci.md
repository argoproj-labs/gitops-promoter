...
- **Codegen verification** — ensures `go.sum`, mockery output (`internal/scms/mock/`), `make build-installer` output (CRDs, `applyconfiguration/`, deepcopy, extension icon styles, `hack/celcost/report.md`, `dist/install.yaml`), and `make generate-ui-types` output (`ui/shared/src/types/generated/crds.gen.ts`) are up to date
