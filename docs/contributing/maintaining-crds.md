...
- Express reconciler RBAC with **`// +kubebuilder:rbac`** on the controller that performs each API call; regenerate manifests rather than editing `config/rbac/role.yaml` by hand. For finalizer-only writes on built-in types (for example Secrets), use `SubResource("finalizers").Update` and grant `secrets/finalizers: update` instead of `update` on the main resource.
