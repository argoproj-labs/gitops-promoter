...
Notes are stored at `refs/notes/promoter.history` on the Git repository. During reconciliation the PromotionStrategyHistory controller reads them and rebuilds `PromotionStrategyHistory.status.history` for the environment.
...
Check `status.history[].mergeCommitSnapshotMismatch` on the environment's PromotionStrategyHistory. When `true`, these fields in that entry are potentially stale:
