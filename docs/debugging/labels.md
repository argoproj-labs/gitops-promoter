...
**Ordering gates:** Promotion ordering CommitStatuses (common keys `dependents-successful` or `dependents-successful`) are created by the [DependentsSuccessfulCommitStatus](../gating-promotions/built-in-gates/dependents-successful-commit-status.md) controller and carry all three standard labels, including `promoter.argoproj.io/dependents-successful-commit-status`. [DependentsSuccessfulCommitStatus](../gating-promotions/built-in-gates/dependents-successful-commit-status.md) generates a chain-shaped `DependentsSuccessfulCommitStatus`; it does not create CommitStatuses directly, so the parent-gate label points at the generated DependentsSuccessfulCommitStatus name.
...
- **Gate controllers** (DAG, Argo CD, timed, web request, git commit, scheduled) set all three standard labels via `utils.CommitStatusStandardLabels(parent, branch, key)` and use the parent-gate label when cleaning up orphaned CommitStatuses. DependentsSuccessfulCommitStatus maintains a child DependentsSuccessfulCommitStatus that performs the CommitStatus writes.
...
List CommitStatuses owned by a DependentsSuccessfulCommitStatus gate (including one generated by DependentsSuccessfulCommitStatus):

```bash
kubectl get commitstatus -n <namespace> -l promoter.argoproj.io/dependents-successful-commit-status=<gate-name>
```
...
3. **Owning gate or policy** — Name of the `DependentsSuccessfulCommitStatus`, `DependentsSuccessfulCommitStatus`, `ArgoCDCommitStatus`, `TimedCommitStatus`, `WebRequestCommitStatus`, `GitCommitStatus`, `PromotionStrategy`, or `ChangeTransferPolicy` involved.
