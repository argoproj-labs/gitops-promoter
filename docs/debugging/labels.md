...
- **Gate controllers** (DAG, Argo CD, timed, web request, git commit) set all three standard labels via `utils.CommitStatusStandardLabels(parent, branch, key)` and use the parent-gate label when cleaning up orphaned CommitStatuses. PreviousEnvironmentCommitStatus maintains a child DAGCommitStatus that performs the CommitStatus writes.
