...
**Webhook secrets** are not fields on the WRCS. Put them on the **Provider secret** already referenced by `ScmProvider.spec.secretRef` / `ClusterScmProvider.spec.secretRef`:
...
**You can use:** `PromotionStrategy` (full spec and status), `WebRequestCommitStatus` (full spec and status snapshot), `Phase` (aggregate of all applicable branches' phases), `TriggerOutput`, `ResponseOutput` (trigger mode), `SuccessOutput`, **`Variables`** (when `when.variables` is set), and `NamespaceMetadata` labels and annotations.
...
Configure the Provider secret with `webhookSecret` (and optionally `webhookSignatureHeader`) so the receiver can verify deliveries. See [Webhook mode](#webhook-mode-inbound-accelerator).
...
spec:
  ...
  success:
    when:
      ...
      expression: >-
        Response != nil ? (Response.StatusCode == 200 && Variables.requireApproval) : Phase == "success"
  mode:
    trigger:
      requeueDuration: 1m
      when:
