...
But when we started to introduce DAG support, we realized things would get complicated quickly if ordering stayed
embedded in PromotionStrategy:
...
spec:
  ...
  # To avoid forcing users to define trivial dependsOn fields for the linear case, we would need a DAG/Linear mode toggle.
  # To support fully-custom ordering, we would also need a None mode, which turns off ordering commit status injection
  # completely.
  order: DAG
```
...
[Dependents Successful Commit Status](gating-promotions/built-in-gates/dependents-successful-commit-status.md) for
