...
func (pr *PullRequest) AddLabels(ctx context.Context, pullRequest promoterv1alpha1.PullRequest, labelNames []string) error {
	...
	labelIDs, err := pr.labelIDsForNames(repo.Spec.Gitea.Owner, repo.Spec.Gitea.Name, labelNames, true)
	if err != nil {
		return err
	}
...
func (pr *PullRequest) RemoveLabels(ctx context.Context, pullRequest promoterv1alpha1.PullRequest, labelNames []string) error {
	...
	labelIDs, err := pr.labelIDsForNames(repo.Spec.Gitea.Owner, repo.Spec.Gitea.Name, labelNames, false)
	if err != nil {
		return err
	}
...
const defaultGiteaLabelColor = "cccccc"
