...
func (pr *PullRequest) Create(ctx context.Context, title, head, base, description string, prObj promoterv1alpha1.PullRequest) (string, error) {
	...
	if resp != nil {
		logger.V(6).Info("gitea response status", "status", resp.Status)
	}
...
// AddLabels adds labels to a pull request on Gitea, creating missing repository labels first.
func (pr *PullRequest) AddLabels(ctx context.Context, pullRequest promoterv1alpha1.PullRequest, labelNames []string) error {
	...
	labelIDs, err := pr.labelIDsForNames(ctx, repo, repo.Spec.Gitea.Owner, repo.Spec.Gitea.Name, labelNames, true)
	if err != nil {
		return err
	}
...
func (pr *PullRequest) RemoveLabels(ctx context.Context, pullRequest promoterv1alpha1.PullRequest, labelNames []string) error {
	...
	labelIDs, err := pr.labelIDsForNames(ctx, repo, repo.Spec.Gitea.Owner, repo.Spec.Gitea.Name, labelNames, false)
	if err != nil {
		return err
	}
...
func (pr *PullRequest) createRepoLabelID(ctx context.Context, gitRepo *promoterv1alpha1.GitRepository, owner, repo, name string) (int64, error) {
	start := time.Now()
	created, resp, err := pr.giteaClient.CreateLabel(owner, repo, gitea.CreateLabelOption{
		Name:  name,
