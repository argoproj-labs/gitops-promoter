...
func (p *BundleProvider) reconcileKey(ctx context.Context, key types.NamespacedName) {
	...
	if apierrors.IsNotFound(err) {
		...
		p.mu.Unlock()
		if wasKnown {
			tombstone := &dashboardapi.PromotionStrategyDetails{
...
func (p *BundleProvider) Get(ctx context.Context, namespace, name string) (*dashboardapi.PromotionStrategyDetails, error) {
...
func (p *BundleProvider) List(ctx context.Context, namespace string) (*dashboardapi.PromotionStrategyDetailsList, error) {
	psList := &promoterv1alpha1.PromotionStrategyList{}
	var listOpts []client.ListOption
	if namespace != "" {
		listOpts = append(listOpts, client.InNamespace(namespace))
	...
	out := &dashboardapi.PromotionStrategyDetailsList{
