...
var _ = Describe("GetShaMetadataFromFile reference sanitization", func() {
	var tempRepoDir string
	var workDir string
	var defaultBranch string
	var repo *v1alpha1.GitRepository
	...
	readMetadata := func(metadata []byte) v1alpha1.CommitShaState {
		GinkgoHelper()
		err := os.WriteFile(filepath.Join(workDir, "hydrator.metadata"), metadata, 0o644)
		Expect(err).NotTo(HaveOccurred())
		_, err = runGitCmd(workDir, "add", "hydrator.metadata")
		Expect(err).NotTo(HaveOccurred())
		_, err = runGitCmd(workDir, "commit", "-m", "Add hydrator metadata")
		Expect(err).NotTo(HaveOccurred())
		_, err = runGitCmd(workDir, "push", "origin", defaultBranch)
		Expect(err).NotTo(HaveOccurred())
		...
		g := git.NewEnvironmentOperations(repo, gap, "default/testrepo")
		Expect(g.CloneRepo(GinkgoT().Context())).To(Succeed())
		Expect(g.LoadCommitAndMetadataBlobs(GinkgoT().Context(), "", headSha)).To(Succeed())

		state, err := g.GetShaMetadataFromFile(GinkgoT().Context(), headSha, "")
		Expect(err).NotTo(HaveOccurred())
		return state
	}

	It("clears only invalid fields and keeps the reference item", func() {
		state := readMetadata([]byte(`{
...
var _ = Describe("sanitizeReferences", func() {
	...
	It("returns nil for a nil refs slice", func() {
		Expect(git.SanitizeReferences(ctx, nil)).To(BeNil())
	})

	It("returns an empty slice for an empty refs slice", func() {
		Expect(git.SanitizeReferences(ctx, []v1alpha1.RevisionReference{})).To(BeEmpty())
	})

	It("skips nil Commit entries without panicking", func() {
		refs := []v1alpha1.RevisionReference{{Commit: nil}}
		result := git.SanitizeReferences(ctx, refs)
		Expect(result).To(HaveLen(1))
		Expect(result[0].Commit).To(BeNil())
	})

	It("clears both invalid fields but keeps other commit metadata", func() {
		refs := []v1alpha1.RevisionReference{
			{
				Commit: &v1alpha1.CommitMetadata{
					Sha:     "not-a-valid-sha",
	...
	It("sanitizes only invalid entries in a mixed refs slice", func() {
		refs := []v1alpha1.RevisionReference{
			{
				Commit: &v1alpha1.CommitMetadata{
					Sha:     sha40,
