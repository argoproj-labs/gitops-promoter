...
func (r *CommitStatusReconciler) Reconcile(ctx context.Context, req ctrl.Request) (result ctrl.Result, err error) {
	...
	logger.V(1).Info("Reconciling CommitStatus", "name", req.Name)
	startTime := time.Now()
	...
	if err != nil {
		if errors.IsNotFound(err) {
			logger.V(1).Info("CommitStatus not found")
	...
	if cs.Spec.Sha == "" || cs.Spec.Phase == "" {
		logger.V(1).Info("Skip setting commit status, missing sha or phase", "sha", cs.Spec.Sha, "phase", cs.Spec.Phase)
		return ctrl.Result{}, nil
	}
...
func (r *CommitStatusReconciler) triggerReconcileChangeTransferPolicy(ctx context.Context, cs promoterv1alpha1.CommitStatus, oldSha, newSha string) error {
	...
	for _, ctp := range ctpList {
		...
		logger.V(1).Info("Reconcile of ChangeTransferPolicy triggered", "ChangeTransferPolicy", ctp.Name,
