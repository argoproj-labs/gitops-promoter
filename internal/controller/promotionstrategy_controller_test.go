...
var _ = Describe("PromotionStrategy Controller", func() {
	Context("When reconciling a resource with no commit statuses", func() {
		...
		It("should successfully reconcile the resource", func() {
			...
			Eventually(func(g Gomega) {
				...
				g.Expect(promotionStrategy.Status.Environments[0].Active.AggregatedCommitStatus.Phase).To(Equal(string(promoterv1alpha1.CommitPhaseSuccess)))
				...
				g.Expect(promotionStrategy.Status.Environments[0].Proposed.AggregatedCommitStatus.Phase).To(Equal(string(promoterv1alpha1.CommitPhaseSuccess)))
				...
				g.Expect(promotionStrategy.Status.Environments[1].Active.AggregatedCommitStatus.Phase).To(Equal(string(promoterv1alpha1.CommitPhaseSuccess)))
				...
				// Success due to PromotionStrategy not having any CommitStatuses configured
				g.Expect(promotionStrategy.Status.Environments[1].Proposed.AggregatedCommitStatus.Phase).To(Equal(string(promoterv1alpha1.CommitPhaseSuccess)))
				...
				g.Expect(promotionStrategy.Status.Environments[2].Active.AggregatedCommitStatus.Phase).To(Equal(string(promoterv1alpha1.CommitPhaseSuccess)))
				...
				// Success due to PromotionStrategy not having any CommitStatuses configured
				g.Expect(promotionStrategy.Status.Environments[2].Proposed.AggregatedCommitStatus.Phase).To(Equal(string(promoterv1alpha1.CommitPhaseSuccess)))
	...
	Context("When reconciling a resource with active commit status using ArgoCDCommitStatus", func() {
		...
		It("should successfully reconcile the resource", func() {
			...
			By("Checking that the AggregatedCommitStatus for each environment is created from ArgoCDCommitStatus")
